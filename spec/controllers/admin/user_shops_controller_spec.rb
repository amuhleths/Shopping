...
describe Admin::UserShopsController do
  ...
  def mock_user_shop(stubs={})
    (@mock_user_shop ||= mock_model(UserShop).as_null_object).tap do |user_shop|
      user_shop.stub(stubs) unless stubs.empty?
    end
  end

  describe "GET index" do
    it "assigns all user_shops as @user_shops" do
      UserShop.stub(:all) { [mock_user_shop] }
      get :index
      assigns(:user_shops).should eq([mock_user_shop])
    end
  end

  describe "GET show" do
    it "assigns the requested user_shop as @user_shop" do
      UserShop.stub(:find).with("37") { mock_user_shop }
      get :show, :id => "37"
  ...
  describe "GET new" do
    it "assigns a new user_shop as @user_shop" do
      UserShop.stub(:new) { mock_user_shop }
      get :new
      assigns(:user_shop).should be(mock_user_shop)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created user_shop as @user_shop" do
        UserShop.stub(:new).with({'these' => 'params'}) { mock_user_shop(:save => true) }
        post :create, :user_shop => {'these' => 'params'}
        assigns(:user_shop).should be(mock_user_shop)
      end

      it "redirects to the created user_shop" do
        UserShop.stub(:new) { mock_user_shop(:save => true) }
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved user_shop as @user_shop" do
        UserShop.stub(:new).with({'these' => 'params'}) { mock_user_shop(:save => false) }
        post :create, :user_shop => {'these' => 'params'}
        assigns(:user_shop).should be(mock_user_shop)
      end

      it "re-renders the 'new' template" do
        UserShop.stub(:new) { mock_user_shop(:save => false) }
        post :create, :user_shop => {}
  ...
  describe "PUT update" do
    ...
    describe "with invalid params" do
      it "assigns the user_shop as @user_shop" do
        UserShop.stub(:find) { mock_user_shop(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:user_shop).should be(mock_user_shop)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the admin_user_shops list" do
      UserShop.stub(:find) { mock_user_shop }
      delete :destroy, :id => "1"
